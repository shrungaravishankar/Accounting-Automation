...
export const handler = async (event: Event) => {
  ...
  try {
    ...
    // Full 4-column trial balance for two periods (previous + current) so the
    // executor self-review can show Opening | Debit | Credit | Closing for each
    // ledger across both months. Opening of a period = cumulative balance as at
    // the day before it starts; period Debit/Credit = movement within the range;
    // Closing = cumulative balance as at period end. RISKY: /reports/* is plan/
    // scope dependent — on failure we return a populated `error` so the frontend
    // marks the trial balance unavailable. params = { curStart,curEnd,prevStart,prevEnd }.
    if (kind === 'trialBalance') {
      let p: any = {};
      try { p = JSON.parse(event.arguments?.params || '{}'); } catch (_) { p = {}; }
      const { curStart, curEnd, prevStart, prevEnd } = p;
      if (!curStart || !curEnd || !prevStart || !prevEnd) {
        return JSON.stringify({ error: 'curStart, curEnd, prevStart and prevEnd are required', accounts: [], apiUsage: lastApiUsage });
      }
      const EARLY = '2000-01-01';
      const dayBefore = (d: string): string => { const dt = new Date(d + 'T00:00:00Z'); dt.setUTCDate(dt.getUTCDate() - 1); return dt.toISOString().slice(0, 10); };
      // Parse a trial-balance response into a name → {account_type,debit,credit,balance} map.
      const parseTB = (j: any): Record<string, any> => {
        ...
        const walk = (node: any): void => {
          ...
          if (typeof node === 'object') {
            ...
            if (name && hasBal && !seen.has(String(name))) {
              const debit = Number(node.debit || 0), credit = Number(node.credit || 0);
              const balance = node.closing_balance !== undefined ? Number(node.closing_balance)
                : node.net_amount !== undefined ? Number(node.net_amount)
                : node.amount !== undefined ? Number(node.amount)
                : debit - credit;
              map[String(name)] = { account_type: node.account_type || node.type || '', debit, credit, balance };
              seen.add(String(name));
            }
            for (const k of Object.keys(node)) { if (node[k] && typeof node[k] === 'object') walk(node[k]); }
          }
      ...
      const tb = async (from: string, to: string) => parseTB(await zohoGet('reports/trialbalance', accessToken, region, {
      ...
      try {
        ...
        return JSON.stringify({ error: null, accounts, apiUsage: lastApiUsage });
      } catch (e: any) {
        return JSON.stringify({ error: e?.message || String(e), accounts: [], apiUsage: lastApiUsage });
      }
    }
